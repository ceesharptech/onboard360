...
8. **Always inspect generated migration SQL before applying it, never run `prisma migrate dev` blind.** Any database object Prisma doesn't natively model in `schema.prisma` — the pgvector HNSW index on `document_chunks` being the known case, added via raw SQL because of the `Unsupported("vector(384)")` column type — is invisible to Prisma's schema diffing and will be silently generated as a `DROP` statement the next time a migration is created, because Prisma reads its absence from `schema.prisma` as drift to correct. This already happened once (Phase 5.4) and was caught and fixed in Phase 5.5 with a standing `pg_indexes` invariant test. The standing discipline going forward: generate every migration with `prisma migrate dev --create-only` first, read the generated SQL file before applying it, and manually remove any statement that would drop a manually-added object (indexes, constraints, or anything else not expressed in `schema.prisma`) before running the migration for real. If a new raw-SQL object is added in any future phase, add or extend an invariant test for it in the same phase, the same way `phase5_5.test.ts` did for the vector index — don't rely on catching the next drop after the fact.
